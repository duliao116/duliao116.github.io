...
def sort1(arr, start, end, partition):
    ...
    if start < end:
        ...
        sort1(arr, start, pivot_index - 1, partition)
        sort1(arr, pivot_index + 1, end, partition)


def sort2(arr, start, end, partition):
    """
    非递归的形式实现快速排序
    :param partition: 分治算法
    :param arr: 待排序数组
    ...
    :param end: 数组的结束下标
    """
    arr_stack: list = [{'start': start, 'end': end}]
    while arr_stack:
        top = arr_stack.pop()
        start_index = top['start']
        end_index = top['end']
        if start_index < end_index:
            pivot_index = partition(arr, start_index, end_index)
            arr_stack.append({'start': start_index, 'end': pivot_index - 1})
            arr_stack.append({'start': pivot_index + 1, 'end': end_index})
...
def partition1(arr, start, end):
    ...
    # 小于基准元素值的边界下标
    mark = start
    # range(1,2)为[1]，所以把end+1，意味从指定数组的的第二个元素到最后一个元素，
    # 第一个元素是基准元素
...
def partition2(arr, start, end):
    ...
    while left != right:
        ...
        # 交换左右两个指针的值，此时arr[right]<=pivot，arr[left]>pivot
        # 将小的交换到左边，大的交换到右边
        if left != right:
            tmp = arr[left]
            arr[left] = arr[right]
...
if __name__ == '__main__':
    ...
    numbers = [4, 7, 3, 5, 6, 2, 8, 1]
    sort2(numbers, 0, len(numbers) - 1, partition2)
    print(numbers)

```
::: tip
快速排序主要就是体现了分治的特点，每次找到的其实是基准元素的位置，基准元素左侧的值都小于基准元素，基准元素右侧的值都大于等于基准元素，直到每个元素都作为基准元素找到了自己的位置，排序就结束了。
:::